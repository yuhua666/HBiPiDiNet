...
from mmcv.cnn.bricks.BiConv import BiConv
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs='+',
        default=[800, 800], #default=1280*800
...
def print_model_with_flops(model,
    ...
    def accumulate_params(self):
        if is_binary_instance(self):
            return self.__params__/32.0
        elif is_supported_instance(self):
...
def binarylinear_flops_counter_hook(module, input, output):
    input = input[0]
    output_last_dim = output.shape[
        -1]  # pytorch checks dimensions, so here we don't care much
    module.__flops__ += int(np.prod(input.shape) * output_last_dim / 64)
...
def binaryconv_flops_counter_hook(conv_module, input, output):
    # Can have multiple inputs, getting the first one
    input = input[0]
...
def is_binary_instance(module):
    if type(module) in get_binary_mapping():
        return True
    return False
...
def get_modules_mapping():
    return {
        ...
        HardBinaryConv: binaryconv_flops_counter_hook,
        BinarizeConv2d: binaryconv_flops_counter_hook,
        BiConv: binaryconv_flops_counter_hook,
        ...
        HardBinaryLinear: binarylinear_flops_counter_hook,
        QuantizeLinear: binarylinear_flops_counter_hook,
        ...
        #bias
        LearnableBias: empty_flops_counter_hook,
    }
