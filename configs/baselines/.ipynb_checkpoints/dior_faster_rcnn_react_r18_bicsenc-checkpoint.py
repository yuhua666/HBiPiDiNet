...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'Horizontal Bounding Boxes_to_coco_train.json',
        img_prefix=data_root + 'JPEGImages-trainval',
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'Horizontal Bounding Boxes_to_coco_test.json',
...
model = dict(
    ...
    neck=dict(
        ...
        conv_cfg = dict(type='BiConv2d_csenc'),
        out_channels=256,
        num_outs=5),
    rpn_head=dict(
        type='RPNHead_React',
...
test_cfg = dict(
    rpn=dict(
        ...
        min_bbox_size=0),
    rcnn=dict(
        score_thr=0.05, nms=dict(type='nms', iou_thr=0.5), max_per_img=3000)
