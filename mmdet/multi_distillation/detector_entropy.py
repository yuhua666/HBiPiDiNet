...
def batch_processor_entropy(model, model_t, data, train_mode, kd_warm=dict(), kd_decay=1., epoch=0, **kwargs):
    ...
    # add entropy
    EntropyLoss = entropy_loss(**data, feat_t=neck_feat_t, proposal_list_t=proposal_list_t,
                               feat_adapt_s=neck_feat, proposal_list_s=proposal_list_s)
    loss_entro, loss_entro_back, loss_entro_beta = EntropyLoss(model, neck_feat_t, neck_feat, kd_cfg)
    losses['loss_entro'] = loss_entro
    if 'entro-back' in kd_cfg.type:
        losses['loss_entro_back'] = loss_entro_back
    if 'entro_beta' in kd_cfg.type:
        losses['loss_entro_beta'] = loss_entro_beta
    # neck_m_batch = get_fpn_entropy_mask(**data, feat_t=neck_feat_t, proposal_list_t=proposal_list_t,
    #                                      feat_adapt_s=neck_feat, proposal_list_s=proposal_list_s)
        ...
        for i, _neck_feat in enumerate(neck_feat):
            ...
            if 'pixel-wise' in kd_cfg.type:
                if 'L1' in kd_cfg.type:
                    ...
                    losskd_neck += (torch.pow(1 - neck_feat_adapt / (neck_feat_t[i] + 1e-8),
                                              2) * mask_hint).sum() / norms
                    # losskd_entro += (torch.pow(1 - neck_feat_adapt / (neck_feat_t[i] + 1e-8),
                    #                           2) * entro_mask).sum() / norms_entro
                    ...
                    # losskd_entro += (torch.pow(neck_feat_adapt - neck_feat_t[i], 2) * entro_mask).sum() / norms_entro
                elif 'neck_hint_all' in kd_cfg.type:
                    B_, C_, H_, W_ = neck_feat_adapt.size()
                    losskd_neck = losskd_neck + (torch.pow(neck_feat_adapt - neck_feat_t[i], 2)).sum() / (B_*C_*H_*W_*2)
                else:
                    losskd_neck = losskd_neck + (torch.pow(neck_feat_adapt - neck_feat_t[i], 2) *
                                                 mask_hint).sum() / norms
                    # losskd_entro = losskd_entro + (torch.pow(neck_feat_adapt - neck_feat_t[i], 2) *
...
def train_detector_entropy(model,
                      model_t,
                      dataset,
                      cfg,
                      distributed=False,
